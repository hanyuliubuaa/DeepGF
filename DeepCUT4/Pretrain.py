...
for epoch in range(num_epochs):
    ...
    for batch_index, (data, labels) in enumerate(Train_loader):
        ...
        for e in range(L):
            ...
            x_CUT4_1 = torch.zeros_like(x_CUT4, device=device)
            for i in range(N):
                x_CUT4_1[:, i, :] = (sqrt_P @ x_CUT4[:, i, :].unsqueeze(-1)).squeeze(-1) + x[:, e, :]
            ...
            loss = loss + (LossF(x_sigma, x_CUT4_1))/(2*L)

            x_pred_sigma = f(x_CUT4_1.reshape([-1, D_x])).reshape([B, -1, D_x]) #[B, N, D_x]
            ...
            x_CUT4_2 = torch.zeros_like(x_CUT4, device=device)
            for i in range(N):
                x_CUT4_2[:, i, :] = (sqrt_P @ x_CUT4[:, i, :].unsqueeze(-1)).squeeze(-1) + x_pred
            ...
            # loss = loss + (LossF(w_sigma, w_CKF5)*10 + LossF(x_sigma, x_CKF5))/(2*L)
            loss = loss + (LossF(x_sigma, x_CUT4_2))/(2*L)

            z_pred_sigma = h(x_CUT4_2.reshape([-1, D_x])).reshape([B, -1, D_z]) #[B, N, D_z]
            ...
            x_centered = x_CUT4_2 - x_pred.unsqueeze(1)
