...
for epoch in range(num_epochs):
    ...
    for batch_index, (data, labels) in enumerate(Train_loader):
        ...
        for e in range(L):
            ...
            for i in range(N):
                x_CKF5[:, i, :] = (sqrt_P @ x_CKF5[:, i, :].unsqueeze(-1)).squeeze(-1) + x[:, e, :]
            ...
            loss = loss + (LossF(x_sigma, x_CKF5))/(2*L)

            x_pred_sigma = f(x_CKF5.reshape([-1, D_x])).reshape([B, -1, D_x]) #[B, N, D_x]
            ...
            for i in range(n):
                ...
                x_CKF5[:, num + 4 * R + n, i] = x_CKF5[:, num + 4 * R + n, i] - torch.sqrt(torch.tensor(n + 2, dtype=torch.float32, device=device))
            for i in range(N):
                x_CKF5[:, i, :] =(sqrt_P @ x_CKF5[:, i, :].unsqueeze(-1)).squeeze(-1) + x_pred
            ...
            z_pred_sigma = h(x_CKF5.reshape([-1, D_x])).reshape([B, -1, D_z]) #[B, N, D_z]
