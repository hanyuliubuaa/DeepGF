...
model_weights_path = './model/DeepCKF5_1.pt'
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_sigma
model = MLP().to(device)
model.load_state_dict(torch.load('./model/UKF_20.pt'))
...
for epoch in range(num_epochs):
    for batch_index, (data, labels) in enumerate(Train_loader):
        ...
        for k in range(L):
            ...
            # loss = loss + torch.mean((residual.transpose(1, 2) @ torch.linalg.inv(RR.repeat(B, 1, 1)) @ residual).squeeze()) / L
