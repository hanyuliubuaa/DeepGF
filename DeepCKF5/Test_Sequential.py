...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_sigma
model = MLP().to(device)
model.load_state_dict(torch.load('./model/DeepCKF5.pt'))
...
with torch.no_grad():
    ...
    for num in range(1000):
        ...
        for k in range(L):
            ...
            w_sigma[:, 0] = w_sigma[:, 0] + 2 / (n + 2)
            w_sigma[:, 1:1 + 4 * R] = w_sigma[:, 1:1 + 4 * R] + 1 / (n + 2)**2
            w_sigma[:, 1 + 4 * R:] = w_sigma[:, 1 + 4 * R:] + (4 - n) / (2 * (n + 2)**2)
