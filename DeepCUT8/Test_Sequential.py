...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_sigma
model = MLP().to(device)
model.load_state_dict(torch.load('./model/DeepCUT8.pt'))
...
with torch.no_grad():
    ...
    w_sigma[:, 6:14] = w_sigma[:, 6:14] + w2
    w_sigma[:, 14:26] = w_sigma[:, 14:26] + w3
    w_sigma[:, 26:34] = w_sigma[:, 26:34] + w4
    w_sigma[:, 34:58] = w_sigma[:, 34:58] + w6
    w_sigma[:, 58] = w7
    for num in range(1000):
        print(num)
        for k in range(L):
            # Time Update Step
            U, S, V = torch.svd(P[num, k, :, :].unsqueeze(0))
            ...
            x_sigma = model(torch.cat((x[num, k, :].unsqueeze(0), sqrt_P_lower), dim=1))
