...
X_TRUE = torch.tensor(np.loadtxt("./data/Train_gt.txt", dtype='float32'), device=device).view(-1, D_x, L+1).transpose(2, 1)
Z_TRUE = torch.tensor(np.loadtxt("./data/Train_data.txt", dtype='float32'), device=device).view(-1, D_z, L).transpose(2, 1)

x_true = X_TRUE
z = Z_TRUE

space = [
    Real(1e-4, 5.0, name='alpha'),
    Real(0.0, 5.0, name='beta'),
    Real(0.0, 5.0, name='kappa'),
]

@use_named_args(space)
def objective(alpha, beta, kappa):
    with torch.no_grad():
        data = z #[B, L, D_z]
        B, _, _ = data.shape
...
print(f"x1 = {res.x[0]:.4f}, x2 = {res.x[1]:.4f}, x3 = {res.x[2]:.4f}")
print(f"Minimum objective value: {res.fun:.4f}")
