...
for epoch in range(num_epochs):
    ...
    for batch_index, (data, labels) in enumerate(Train_loader):
        ...
        for k in range(L):
            ...
            for i in range(D_x):
                x_CKF[:, i, :] = x[:, k, :] + np.sqrt(D_x) * sqrt_P[:, :, i]
                x_CKF[:, i+D_x, :] = x[:, k, :] - np.sqrt(D_x) * sqrt_P[:, :, i]
            ...
            # loss = loss + (LossF(w_sigma, w_CKF)*10 + LossF(x_sigma, x_CKF))/(2*L)
            loss = loss + (LossF(x_sigma, x_CKF))/(2*L)
